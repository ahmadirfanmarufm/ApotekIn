...
function mapPoItemsToForms(items: PurchaseOrderDetailItem[], startKey: number, restockItemId?: string | null, restockQuantity?: number | null): ReceiptItemForm[] {
  const availableItems = items.filter((item) => item.remainingQty > 0);

  // Jika modal dibuka dari tombol Restock,
  ...
  // Default: tampilkan semua item PO yang masih bisa diterima
...
function IncomingStockModalContent({ onClose, onSuccess, restockItemId, restockQuantity }: IncomingStockModalProps) {
  ...
  useEffect(() => {
    ...
    const loadPoDetail = async () => {
      ...
      try {
        ...
        const mappedItems = mapPoItemsToForms(data.data.items, 1, restockItemId, restockQuantity);
        ...
        if (restockItemId) {
          const restockItemExists = data.data.items.some(
