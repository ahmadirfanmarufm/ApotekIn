...
export async function GET() {
  try {
    ...
    console.error("GET COMPOUND ITEMS ERROR:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Terjadi kesalahan saat mengambil data bahan racikan.",
      },
      { status: 500 },
    );
  }
...
export async function POST(request: Request) {
  try {
    ...
    const { name, unit, minStock, maxStock, description, batch } = body;
    ...
    const { batchNumber, quantity, expiryDate, buyPrice, sellPrice } = batch;
    ...
    if (existingBatch) {
      return NextResponse.json(
        {
          message: "Nomor batch tersebut sudah digunakan.",
        },
        { status: 409 },
      );
    }
