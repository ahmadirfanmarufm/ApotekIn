...
export function CompoundItemModal({
  ...
  const [selectedBatch, setSelectedBatch] = useState<CompoundBatch | null>(
    null,
  );
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        isEdit
          ? `/api/inventory/compound/${item?.id}`
          : "/api/inventory/compound",
        {
  ...
  return (
    <div
      ...
      onMouseDown={() => !loading && onOpenChange(false)}
    >
      <div
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-manrope text-lg font-bold text-slate-900">
                {isEdit ? "Edit Bahan Racikan" : "Tambah Bahan Racikan"}
              </h2>

              <p className="text-xs text-slate-400">
                Kelola informasi bahan dan stok batch
              </p>
            </div>
          </div>
            ...
            <section className="rounded-2xl border border-violet-100 bg-violet-50/40 p-4 sm:p-5">
              <div className="mb-4">
                ...
                <p className="mt-1 text-xs leading-5 text-slate-400">
                  Masukkan stok batch pertama saat bahan dicatat ke dalam
                  sistem.
                </p>
              </div>
                  ...
                  item.batches.map((batch) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-slate-500">
                          Beli: Rp{" "}
                          {Number(batch.buyPrice).toLocaleString("id-ID")}
                        </p>

                        <p className="text-sm text-slate-500">
                          Jual: Rp{" "}
                          {Number(batch.sellPrice).toLocaleString("id-ID")}
                        </p>
                      </div>
