...
export default async function OtcBatchDetailPage({ params }: OtcBatchDetailPageProps) {
  ...
  const serializedItem = {
    ...
    batches: item.batches.map((batch) => ({
      ...
      stockOutItems: batch.stockOutItems.map((stockOutItem) => ({
        ...
        stockOut: {
          ...
          createdAt: stockOutItem.stockOut.createdAt.toISOString(),
        },
      })),
    })),
    ...
    purchaseOrderItems: item.purchaseOrderItems.map((poItem) => ({
      ...
      expiryDate: poItem.expiryDate ? poItem.expiryDate.toISOString() : "",
      ...
      purchaseOrder: {
        ...
        receivedAt: poItem.purchaseOrder.receivedAt?.toISOString() ?? null,
  ...
  return (
    <div className="space-y-6 pb-12 font-inter text-slate-800">
      <div className="flex items-center gap-2 text-sm text-slate-400">
        <Link href="/inventory/otc" className="transition hover:text-slate-700">
          Obat OTC
        </Link>

        <span>/</span>

        <span className="font-medium text-slate-600">Detail Batch</span>
      </div>

      <OtcBatchDetail item={serializedItem} />
    </div>
  );
}
